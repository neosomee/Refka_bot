...
from app.db import get_user, create_db, init_bonus_settings, update_user_balance, create_users_table, add_withdrawal, get_user_balance, get_withdrawals, get_saved_payeer_id, save_payeer_id, create_payeer_wallets_table
...
# Конфигурация
TOKEN = 'ТВОЙ ТОКЕН'
CHANNEL_ID = "ТВОЙ ID КАНАЛА"
ADMIN_ID = "ТВОЙ ID ТЕЛЕГРАММ АККАУНТА"
...
# Обработчик просмотра заявок на вывод средств
...
async def view_withdrawals_handler(callback: types.CallbackQuery):
    try:
        ...
        if not withdrawals:
            await callback.message.answer("Нет активных заявок на вывод средств.")
            return
