...
async def handle_sub_channel(callback: types.CallbackQuery, bot: Bot, *, channel_id):
    ...
    try:
        ...
        if user and user.get("subscription_bonus_received", False):
            await callback.answer("Ты уже был давно подписан, бонус начислен давно.", show_alert=True)
            return
...
async def handle_referral_program(message: types.Message):
    ...
    if user:
        ...
        bot_name = "ТВОЯ ССЫЛКА НА БОТА БЕЗ СОБАКИ ПРИМЕР: testik_ref"
